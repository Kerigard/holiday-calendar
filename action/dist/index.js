...
exports.createCalendar = exports.getHolidays = exports.getYears = void 0;
const http_client_1 = __nccwpck_require__(6255);
const blueimp_md5_1 = __importDefault(__nccwpck_require__(1403));
const ical_generator_1 = __importStar(__nccwpck_require__(3658));
const xml2js_1 = __nccwpck_require__(6189);
const getYears = async () => {
    const response = await new http_client_1.HttpClient().get('http://xmlcalendar.ru/data/ru/all/calendar.json');
...
const getHolidays = async (year) => {
    const response = await new http_client_1.HttpClient().get(`http://xmlcalendar.ru/data/ru/${year}/calendar.xml`);
    if (response.message.statusCode !== 200) {
        throw new Error(response.message.statusMessage);
    }
    const xml = await (0, xml2js_1.parseStringPromise)(await response.readBody());
    const days = [];
    const data = [];
    const holidays = [];
    ...
    for (const [key, holiday] of Object.entries(data).reverse()) {
        ...
        if (addDays(holiday.started_at, 1).toISOString() !== holidays[index]?.started_at.toISOString() ||
...
const createCalendar = (holidays) => {
    ...
    for (const holiday of holidays) {
        calendar.createEvent({
            ...
            id: (0, blueimp_md5_1.default)(`${holiday.title}${holiday.started_at.toISOString().slice(0, 10)}${holiday.ended_at.toISOString().slice(0, 10)}`),
            stamp: new Date(`${holiday.year}-01-01T00:00:00Z`),
            start: holiday.started_at,
            end: addDays(holiday.ended_at, 1),
            allDay: true,
            transparency: ical_generator_1.ICalEventTransparency.TRANSPARENT
        });
    }
...
const calendar_1 = __nccwpck_require__(4810);
...
async function run() {
    try {
        ...
        let allHolidays = [];
        const changes = [];
        for (const year of await (0, calendar_1.getYears)()) {
            const holidays = await (0, calendar_1.getHolidays)(year);
            const calendar = (0, calendar_1.createCalendar)(holidays);
            allHolidays = allHolidays.concat(holidays);
            const filename = (0, path_1.resolve)(`${dir}/../years/holidays_${year}.ics`);
            const currentCalendar = (0, fs_1.existsSync)(filename) ? (0, fs_1.readFileSync)(filename).toString() : null;
            if (currentCalendar !== calendar) {
                (0, fs_1.writeFileSync)(filename, calendar);
                changes[year] = currentCalendar ? 'update' : 'create';
            }
        }
        if (Object.keys(changes).length === 0)
            return;
        const calendar = (0, calendar_1.createCalendar)(allHolidays);
        (0, fs_1.writeFileSync)((0, path_1.resolve)(`${dir}/../holidays.ics`), calendar);
...
`),this.data.events.forEach(i=>t+=i.toString()),t+=E(this.data),t+="END:VCALENDAR",N(t)}};function nt(n){return new D(n)}var at=nt;0&&(0);
